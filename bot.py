...
async def handle_session_file(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    msg = await update.message.reply_text("⏳ <b>Конвертирую...</b>\n\nSession → TData", parse_mode="HTML")
    try:
        ...
        await update.message.reply_text(f"❌ <b>Ошибка конвертации</b>\n\n<code>{e}</code>", parse_mode="HTML")
    finally:
...
async def handle_phone(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await update.message.reply_text(f"❌ <b>Ошибка отправки кода</b>\n\n<code>{e}</code>", parse_mode="HTML")
        shutil.rmtree(user_dir, ignore_errors=True)
        return ConversationHandler.END
...
async def handle_code(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        return await finish_session(update, ctx, uid)
        ...
        await update.message.reply_text("❌ <b>Неверный код</b>\n\nПопробуйте ещё раз:", parse_mode="HTML")
        return CODE
    except Exception as e:
...
async def handle_password(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await update.message.reply_text(f"❌ <b>Неверный пароль</b>\n\n<code>{e}</code>", parse_mode="HTML")
        await cleanup(uid)
        return ConversationHandler.END


async def finish_session(update: Update, ctx: ContextTypes.DEFAULT_TYPE, uid: int):
    data = tg_clients[uid]
    client: TelegramClient = data["client"]
    ...
    try:
        ...
        await update.message.reply_text(f"⚠️ <b>tdata не создана</b>\n\n<code>{e}</code>", parse_mode="HTML")
